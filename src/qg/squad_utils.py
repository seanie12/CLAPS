...
from transformers.tokenization_bert import whitespace_tokenize, BasicTokenizer
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in tqdm(enumerate(examples), total=len(examples)):
        ...
        max_tokens_for_doc = max_seq_length - len(start_context_toks)
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            tokens = deepcopy(start_context_toks)
            ...
            q_ids.insert(0, 0)  # 0 for bos_id
            q_ids.append(1)  # 1 for eos_id

            # pad up to maximum length
            decoder_input_ids = q_ids[:-1]
            lm_labels = q_ids[1:]
            decoder_attention_mask = [1] * len(decoder_input_ids)
            # -100 is ignore_idx for cross entropy
            ...
            assert len(decoder_input_ids) == len(decoder_attention_mask)
            assert len(decoder_input_ids) == len(lm_labels)
...
def read_squad_examples(input_file, is_training, version_2_with_negative=False,
    ...
    if debug:
        ...
        input_data = input_data[:int(num_data * ratio)]
